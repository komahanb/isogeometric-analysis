...
\end{figure}
  ...
  \newpage
  \begin{minipage}[b]{0.48\linewidth}
    \includegraphics[width=1.0\textwidth]{doc/figures/undeformed_plate.png} \\
    \centering \label{Underformed Plate}
  \end{minipage}
  \begin{minipage}[b]{0.48\linewidth}
    \includegraphics[width=1.0\textwidth]{doc/figures/deformed_plate.png} \\
    \centering \label{Deformed Plate}
  \end{minipage}
  \begin{itemize}
      \item The nodal displacement vector is obtained using the following equation.
      \item $ [K^G]$\{U\}=$\{F^G\}$ 
      \item The displacements are then added to the control points and the new control points are then used to plot the deformed plate as shown in Fig. \ref{Deformed Plate}
  \end{itemize}
  \newpage
\end{frame}
